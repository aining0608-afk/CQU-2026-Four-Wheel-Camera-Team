...
//    float EFF[7]={-18.5,-13,-6.5,0,6.5,13,18.5};//摄像头误差分区
//    float EFF[7]={-20,-15,-10,0,10,15,20};//摄像头误差分区
    float EFF[7]={-24,-15,-11,0,11,15,24};//摄像头误差分区
    /*输入量D语言值特征点*/
    float DFF[7]={-12,-7.5,-5.5,0,5.5,7.5,12};//误差变化率分区
    /*输出量U语言值特征点(根据赛道类型选择不同的输出值)*/
//    float UFF[7]={0,0.65,1.3,1.95,2.6,3.25,3.9};      //限幅分区
    float UFF[7];//={0,0.3,0.7,1.1,1.6,1.9,2.3};
    int rule[7][7]={
    //    0   1   2   3   4   5   6
        { 6 , 5 , 4 , 3 , 2 , 1 , 0},//0
        { 5 , 4 , 3 , 2 , 1 , 0 , 1},//1
...
{
    /*输入量P语言值特征点*/
//    float EFF[7]={-27,-20,-7,0,7,20,27};//摄像头误差分区
    float EFF[7]={-24,-15,-11,0,11,15,24};//摄像头误差分区
    /*输入量D语言值特征点*/
    float DFF[7]={-12,-7.5,-6,0,6,7.5,12}; //误差变化率分区
    /*输出量U语言值特征点(根据赛道类型选择不同的输出值)*/
//    float UFF[7]={0,0.25,0.5,0.75,1,1.25,1.5};   //限幅分区
    float UFF[7];//={0,0.2,0.4,0.6,0.8,1,1.2};   //限幅分区
