...
uint32 system_ms=0;  //全局运行时间

uint16 fps_cnt[3] = {0, 0, 0};  //图像帧率统计
...
void PIT_init(void)
{
    ...
    pit_init(CCU60_CH0, 1*1000);//1ms中断
    pit_init(CCU60_CH1, 2*1000);//2ms中断
    pit_init(CCU61_CH0, 5*1000);//5ms中断
    pit_init(CCU61_CH1, 100*1000);//100ms中断
}
...
#if(Double_Loop)
    Get_Current();
    L_I_Loop(L_SpeedOutput);
    R_I_Loop(R_SpeedOutput);
    ALL_Out();
#endif
...
IFX_INTERRUPT(cc60_pit_ch1_isr, 0, CCU6_0_CH1_ISR_PRIORITY)
{
    ...
    // === AUTO-START: P33_13 dipswitch gate removed; system_ms increments unconditionally ===
    // (To restore dipswitch run-enable, uncomment the if-block below and remove the bare ++)
    // if(gpio_get_level(P33_13))   /* V2.5 SW5-B run-enable */
    // {
         system_ms++;
    // }
    Get_Attitude();
